...
function rollout(src, iAmLeft, touches) {
  ...
  for (var f = 0; f < TUNE.ROLLOUT_FRAMES; f++) {
    ...
    // 고정 정책: 낙하지점 옆으로 걸어간다(정중앙은 피한다). 점프도 타격도 안 한다.
    var aim = w.ball.expectedLandingPointX - toNet * TUNE.AIR_CHASE_OFFSET;
    var d = aim - me.x;
    var mx = Math.abs(d) > TUNE.MOVE_DEADBAND ? (d > 0 ? 1 : -1) : 0;
    var r = advance(w, iAmLeft, mx, 0, 0);
    ...
    if (r.ground) {
      ...
      return mine ? -1000 + f : 1000 - f;
    }
  }
  // 지평선 안에 결말이 안 났다. 이때가 평가함수가 실력을 결정하는 지점이다.
  //
  // 공이 어느 쪽에 있느냐만 보면 "일단 넘기기"에 머문다. 대신 [5]의 여유를 쓴다:
  // 지금 공 상태에서 낼 수 있는 최선의 공격이 얼마나 위협적인가. 이러면 탐색이
  // "받아넘기기"가 아니라 "못 받는 공을 만드는 자세"를 향하게 된다.
  ...
  if (oppSide) return base;   // 이미 넘어갔으면 조준할 공이 없다
  var opp = iAmLeft ? w.p2 : w.p1;
  var shot = bestShot(w.ball, opp.x, iAmLeft);
  return base + TUNE.MARGIN_WEIGHT * Math.max(-100, Math.min(100, shot.margin));
}
 ...
 */
function search(w, iAmLeft, depth, touches, wasColliding) {
  // 예산은 두 가지로 잰다.
...
function strategyDecide(s) {
  ...
  for (var d = 1; d <= TUNE.MAX_SEARCH_DEPTH; d++) {
    var r = search(base, iAmLeft, d, G.myTouches, colliding);
    if (SEARCH.aborted) break;      // 이 깊이는 미완성 -- 직전 결과를 쓴다
    best = r;
  }
  return best !== null && best.action !== null ? best.action : { x: 0, y: 0, hit: 0 };
}
