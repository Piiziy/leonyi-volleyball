...
//                                       (예산 초과는 무입력 처리된다)
//  ★ 네트 앞 짧은 스매시에 다 뚫린다     [1] STAND_FORWARD_BIAS 올리기
//    (또는 반대로 뒤가 자꾸 빈다)         이 봇에서 가장 크게 작동하는 값이다.
//                                       공을 기다릴 때 네트 쪽으로 몇 px 나와
//                                       설지. 상대 6종에게 각 180세트로 재니
//                                       0=81.7% 40=92.8% 60=95.6% 100=88.9%.
//                                       지금 60(정점). 뒤가 비면 내리고,
//                                       앞이 뚫리면 올린다. 20씩 움직일 것.
...
var TUNE = {
  ...
  //   이 값으로 그 차이를 메운다.
  // ★ 이 봇에서 가장 크게 작동하는 값. 서로 다른 상대 6종에게 각 180세트씩
  //   붙여 고른 것이다(panel.js). 단조증가하다 60 에서 꺾인다:
  //     0=81.7%  20=90.0%  40=92.8%  60=95.6%  80=93.9%  100=88.9%
  //   같은 봇끼리 붙이는 스윕(hsweep.js)은 이 값을 못 고른다 -- 거의 같은 두
  //   봇은 나비효과로 한쪽이 전승해서 0/20/40/60 이 16.7/4.4/98.9/88.9% 같은
  //   엉뚱한 값이 나온다.
  STAND_FORWARD_BIAS: 60,    // 네트 쪽으로 당길 px
  ...
  //
  //   ★ 재 봤더니 해롭다 (상대 6종 각 180세트): 0=93.3% ±3.6  1=80.0% ±5.8.
  //     어디서 잃는지가 이유를 말해 준다 -- 강한 상대에게만 무너진다
  //     (v10 46.7%, v7 60.0%. 약한 상대는 둘 다 100%). 강한 상대는 깊은 공도
  //     섞어 쓰는데, 그걸 보고 물러나면 전진 수비의 이득을 스스로 반납한다.
  //     **전진 수비는 조건 없이 유지해야 한다.** 0 으로 둔다.
  STAND_ADAPT: 0,            // ★ 켜지 말 것. 위 측정 참고.
  ...
  // 예산이 남는 동안 계속 깊이 판다.
  // ★ 강함과는 무관하다는 것이 측정됐다. 서로 다른 상대 6종에게 각 180세트씩
  //   붙였을 때 400=95.6%  1500=95.0%  3000=97.2%  6000=93.3% 로 **전부 구분
  //   불가**다. 그래서 이 값은 실력이 아니라 **안전**을 기준으로 고른다 --
  //   낮을수록 느린 PC 에서 하드 타임아웃(360ms) 여유가 커지고, 15틱 연속
  //   타임아웃으로 워커가 재시작되어 전역 상태가 전부 날아가는 최악을 피한다.
  //   실측(유휴 상태, 같은 상대): 최대 decide 48.9ms -> 36.6ms.
  //   느린 PC 에서는 이 여유가 더 크게 듣는다 -- 한 노드가 비싸질수록
  //   시계를 보는 간격(32노드) 사이에 지나가는 시간이 길어지기 때문이다.
  NODE_BUDGET: 3000,        // ★ 주 제어. 한 번의 decide()가 펼칠 최대 탐색 노드 수.
  ...
  // ★ 측정 완료 — 켠다. v16 기준 30세트 스크리닝에서 80.0% ±14.3 (대조 50.0%).
  //   끄면 시뮬 속 선수가 다이빙을 전혀 안 해서 양쪽 수비 범위를 과소평가한다.
  // --- 서브 ★ 이 봇에서 가장 크게 작동하는 변경 -----------------------------
  //   서브를 탐색에 맡기면 높이 136 에서 수평으로 나가는 공이 되고, 받는 쪽이
  //   20프레임 동안 자리를 잡고 기다렸다가 내려꽂는다. 손으로 정한 로브 서브가
  //   압도적으로 낫다 (serveTest.js, v14 자기대전 10세트):
  //     기본(탐색)  서브측 랠리 승률  3.6% ±4.0 · 세트 승률  50.0%
  //     로브 서브                   54.1% ±11.4 · 세트 승률 100.0%
  SERVE_LOB: 1,              // 0이면 옛 동작(탐색이 서브를 정한다)
  SERVE_ALIGN_X: 6,          // 공과 이만큼 어긋나면 먼저 걸어가 맞춘다
  SERVE_JUMP_BALL_Y: 60,     // 공이 이보다 내려오면 점프한다
  SERVE_HIT_REACH: 40,       // 공중에서 공과 세로로 이 안이면 파워히트

  SIM_DIVE: 0,               // v18 은 v14 동작으로 되돌린다(랠리 중 움직임 유지)
  ...
  // ★ 측정 완료 — 켠다. v14(=0) 상대 **100/100** (ab.js, 100세트, 좌우 스왑,
  //   대조군 50/100 으로 하네스 건전 확인). 끄면 시뮬 속 선수가 타격으로 공의
  //   방향이 바뀐 뒤에도 **낡은 낙하지점**을 향해 계속 뛴다. 롤아웃이 사실상
  //   평가함수이므로(95% 가 지평선 전에 끝난다) 이 오차가 평가 전체를 오염시켰다.
  SIM_LANDING_REFRESH: 0,    // 랠리 중에는 v14 그대로 둔다
  // ★ 서브 구간에서만 위 갱신을 켠다 (v18).
  //   사용자가 브라우저 실전에서 v14 대 v16 을 보고 "초기 서브 빼고는 v14 의
  //   움직임이 낫다"고 관찰했다. 승률만 보면 v16 이 100/100 이지만, 이 레포는
  //   실전 관찰이 측정을 뒤집은 전례가 있다(v4 철회). 그래서 이득이 확실한
  //   구간에만 적용한다 -- 서브는 자책 안 나는 수가 사실상 둘뿐이라 선택지가
  //   좁고, 낙하지점 갱신의 이득이 랠리 중 움직임을 건드리지 않고 나타난다.
  SIM_LANDING_REFRESH_ON_SERVE: 1,
  ...
  // ★ 시뮬 속 선수들이 엔진과 같은 리듬으로 결정하는가.
  //   엔진은 모든 봇에게 TICK_FRAMES(3) 프레임마다 한 번만 입력을 받는다.
  //   0이면 시뮬 속 선수가 매 프레임 새로 결정해 **실제보다 3배 민첩**해진다.
  //   그 오차는 비행시간에 비례해 커진다(느린 공일수록 더 많이 따라잡는다).
  //   그래서 평가가 짧고 평평한 공에 체계적으로 쏠린다 -- attack.js 실측으로
  //   봇이 고른 각도는 수평 57.1% 인데 정답은 위로 올려치기 39.1% 였다.
  // ★ 측정 완료 — 켠다. v16 기준 30/30 (예산 600 스크리닝).
  //   주의: v14 위에서는 71%, 낡은 패널에서는 회귀로 보였다. SIM_LANDING_REFRESH
  //   가 고쳐지고 나서야 제 값을 한다 — 두 오차가 서로를 가리고 있었다.
  SIM_TICK_CADENCE: 0,       // v18 은 v14 동작으로 되돌린다
  ...
  // --- 수 섞기 (상대의 예측표 무력화) --------------------------------------
  // ★ 스매시 각도의 동점을 어느 쪽으로 깰 것인가.
  //   search() 는 엄격한 > 로 최대를 고르므로 동점이면 candidateActions 가
  //   넣은 순서가 이긴다. 그 순서가 [0,-1,1] 이라 **수평이 항상 먼저**였다.
  //   "중립 먼저"라는 근거는 이동 축(x)에서는 타당하지만, 각도 축에서 y=0 은
  //   중립이 아니라 그냥 평평한 강타다.
  //   실측(whynot.js, v14 1세트 111틱): 수평과 위로의 점수가 **73.0% 의 틱에서
  //   정확히 동점**이고 탐색이 최고점을 준 각도는 위로 45.9% / 수평 47.7% 로
  //   거의 같다. 그런데 실제로 두는 것은 수평 57.1% / 위로 9.6% (attack.js).
  //   즉 평가는 구분하지 않는데 목록 순서가 전부 수평으로 몰아주고 있었다.
  //   같은 attack.js 가 말하는 정답 분포는 위로 39.1% / 수평 18.4% 다.
  SMASH_ANGLE_UP_FIRST: 0,   // 1이면 각도 후보를 [-1,0,1](위로 먼저)로 넣는다
