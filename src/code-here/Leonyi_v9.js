...
function pickFromPool(best, s, iAmLeft) {
  ...
  if (TUNE.TIEBREAK_TOWARD_LANDING === 1 && s !== undefined) {
    ...
    for (var i = 0; i < pool.length; i++) {
      ...
      // 이 수를 3프레임 유지하면 x 가 어디로 가는가 (걷기 6px/프레임)
      var future = s.self.x + pool[i].action.x * 6 * TICK_FRAMES;
      var gap = Math.abs(target - future);
      // 헛다이빙·헛점프를 막기 위해, 같은 거리면 조작이 적은 쪽을 택한다
      var penalty = pool[i].action.hit * 2 + (pool[i].action.y === -1 ? 1 : 0);
      var cost = gap + penalty * TUNE.TIEBREAK_ACTION_PENALTY;
      if (cost < pickGap) { pickGap = cost; pick = pool[i].action; }
    }
