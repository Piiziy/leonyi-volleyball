...
| 파일 | 내용 |
|---|---|
| **`src/code-here/Leonyi_v3.js`** | **제출용.** 팀명이 정해지면 이 파일 이름만 바꾼다 |
| `src/code-here/Leonyi_v1.js` | 회귀 비교 기준선. v3 가 이것을 300세트 59.0% ±5.6 으로 이긴다 |
| `LabA_v1.js` `LabC_v1.js` | 전략 비교용 상대 (전탐색이 아닌 다른 설계) |

빌드: `node build.js B --as Leonyi_v3` (전략 B = 전탐색). `LabB_v1.js` 는 빌드
산출물이라 커밋하지 않는다.
