...
| `src/code-here/Leonyi_v10.js` | v7 대비 59.2% ±8.8 |
| `src/code-here/Leonyi_v7.js` | v1 대비 83.3% ±6.7 |
| `src/code-here/Leonyi_v1.js` | 회귀 비교 기준선 |
| `LabA_v1.js` `LabC_v1.js` | 전략 비교용 상대 (전탐색이 아닌 다른 설계) |

빌드: `node build.js B --as Leonyi_v16` (전략 B = 전탐색). `LabB_v1.js` 는 빌드
산출물이라 커밋하지 않는다.

## v14 → v16 은 상수 하나다: `SIM_LANDING_REFRESH` 0 → 1

원저자가 "미측정. 켜기 전에 반드시 재볼 것"으로 남겨 둔 플래그였다. 꺼져 있으면
탐색·롤아웃 안에서 **타격으로 공의 방향이 바뀐 뒤에도 시뮬 속 선수들이 낡은
낙하지점을 향해 계속 뛴다.** 롤아웃의 95%가 지평선 전에 끝나므로 롤아웃 정책이
사실상 평가함수인데, 그 안의 두 선수가 엉뚱한 곳으로 뛰고 있었던 것이다.

    ab.js  v16 vs v14   100/100   (대조군 50/100 — 하네스 건전)
    run.js v16 vs v14   8승 0패, 평균 스코어 10.00 : 0.13

**★ 제출 전에 남은 검증** — 아직 안 돌렸다:
`anomaly.js` · `sanity.js` · `panel.js` 회귀 · 브라우저 실기 확인.

### 이 측정에서 배운 것 (도구를 믿기 전에 읽을 것)

1. **천장에 닿은 패널로 판단하지 말 것.** v14 는 패널 6종 전부에게 180/180 이라
   개선이 원리적으로 안 보인다. 실제로 같은 변경이 패널에서는 "구분 불가",
   v14 직접 대전에서는 100/100 이었다. 그래서 `ab.js`(기준 봇 직접 대전)를
   만들었다. 기준은 **현 챔피언**이어야 한다.
2. **부하 중에 잰 시간은 비용이 아니다.** 병렬 측정 11개가 도는 중에 재니
   v16 의 decide 가 mean 34ms / max 80ms 로 나와 "비용 회귀"로 보였다. 유휴
   상태에서는 **mean 10.9ms / max 79ms** 다. `budget.js` 로 예산별로 잰다.
3. **긴 측정은 결과를 조합마다 파일에 쓸 것.** Node 는 stdout 이 파이프면
   버퍼링해서, 6시간짜리를 돌리고도 끝난 4개 조합의 결과를 못 봤다.
   `ab.js` 는 `appendFileSync` 로 즉시 남긴다.
4. **비용을 먼저 계산할 것.** 세트당 CPU ≈ (양쪽 decide 평균 ms) × 약 4100 호출.

**v10 → v14 는 상수 두 개다.** 탐색 로직은 한 줄도 안 고쳤다.

| 상수 | 값 | 근거 |
|---|---|---|
| `STAND_FORWARD_BIAS` | 0 → **60** | 공을 기다릴 때 네트 쪽으로 나와 서는 px. 상대 6종 각 180세트: 0=81.7% 40=92.8% **60=95.6%** 80=93.9% 100=88.9% |
| `NODE_BUDGET` | 6000 → **3000** | 강함은 구분 불가(400~6000 전부). 안전을 위해 낮췄다 — 최대 decide 48.9ms → 36.6ms |

**측정할 때 주의.** 같은 파일끼리 붙이는 `hsweep.js` 로는 이 값들을 고를 수
없다. 거의 같은 두 봇은 나비효과로 한쪽이 전승해서, 같은 `STAND_FORWARD_BIAS`
가 0/20/40/60 에서 16.7/4.4/98.9/88.9% 같은 엉뚱한 값을 낸다. **`panel.js`**
(서로 다른 상대 여럿에게 붙여 합계)를 쓸 것.
...
승률만 보면 "나빠졌다"까지만 알 수 있고, 자기 대전은 **양쪽이 공유하는 실수를
못 잡는다**(실제로 그래서 v4 를 개선이라고 오판했다). 상대가 필요 없는 검사를
먼저 돌리는 편이 빠르다.

| 도구 | 무엇을 보는가 |
|---|---|
| `anomaly.js` | 사람이 보면 "저건 이상한데" 할 행동을 센다(헛다이빙, 자책, 정지) |
| `sanity.js` | 롤아웃 정책이 자기 코트에 스매시를 꽂지 않는지 |
| `coverage.js` | 넘어오는 공 격자를 전수 조사해 **수비 구멍**을 찾는다 |
| `stand.js` | 대기 위치를 옮겨 가며 커버리지를 재서 **어디에 서야 하는지** |
| `attack.js` | 접촉마다 모든 수를 다시 쳐 보고 **더 좋은 공이 있었는지** |

**coverage.js 를 쓸 때의 함정 둘** (둘 다 없는 구멍을 만들어 냈다):
- 봇 인스턴스를 시나리오 간에 재사용하면 안 된다. 시나리오가 바뀔 때 공이
  순간이동하는 것을 봇의 물리 감시견이 "엔진이 바뀌었다"로 읽고 안전 모드에
  빠진다. 매번 새로 컴파일한다.
- **내장 AI 를 "받을 수 있었는가"의 기준으로 쓰면 안 된다.** AI 는 physics 안에서
  매 프레임 지연 없이 움직이는데, 제출 봇은 3프레임마다 결정 + 1프레임 지연이라
  최대 18px 을 먼저 잃는다. AI 만 받는 공은 대개 그 18px 차이다.
