...
| 파일 | 내용 |
|---|---|
| **`src/code-here/Leonyi_v12.js`** | **제출용.** v10 대비 89.2% ±5.6 · v7 대비 90.0% ±5.9 · v1 대비 80.0% ±7.8 |
| `src/code-here/Leonyi_v10.js` | 직전 제출용. v7 대비 59.2% ±8.8 |
...
빌드: `node build.js B --as Leonyi_v12` (전략 B = 전탐색). `LabB_v1.js` 는 빌드
산출물이라 커밋하지 않는다.

**v10 → v12 는 상수 하나다.** `STAND_FORWARD_BIAS: 0 → 40` (공을 기다릴 때
네트 쪽으로 나와 서는 px). 탐색을 한 줄도 고치지 않았다. 이 세션에서 만든
어떤 탐색 개선보다 컸다 — 자세한 근거는 EXPERIMENTS.md.
